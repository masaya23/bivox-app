...
// テキストファイルをパース
const txt = fs.readFileSync('data/unit3_sentences.txt', 'utf-8');
...
for (const line of lines) {
  const idMatch = line.match(/^(unit3-p\d+-s\d+)\s+Tags:\s*(.+)/);
  if (idMatch) {
    currentId = idMatch[1];
...
// JSON読み込み
const json = JSON.parse(fs.readFileSync('data/units/junior-high-3/unit3.json', 'utf-8'));
...
// JSON書き込み
fs.writeFileSync('data/units/junior-high-3/unit3.json', JSON.stringify(json, null, 2), 'utf-8');
