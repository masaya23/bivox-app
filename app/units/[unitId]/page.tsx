...
export default function UnitDetailPage({ params }: { params: Promise<{ unitId: string }> }) {
  ...
  const initialShuffleMode = searchParams.get('shuffle') === 'true';

  const unit = getUnitById(resolvedParams.unitId);
  const [shuffleMode, setShuffleMode] = useState(initialShuffleMode);
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="space-y-4">
          {unit.parts.map((part) => (
            <Link
              key={part.id}
              href={`/units/${unit.id}/parts/${part.id}?shuffle=${shuffleMode}`}
